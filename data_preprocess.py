    ...
    if not os.path.exists('data/{}.bin'.format(dataset)):
        if not os.path.exists('data/{}.csv'.format(dataset)):
            if not os.path.exists('data'):
                os.mkdir('data')
            if dataset in ['mooc','wikipedia']:
                url = 'https://snap.stanford.edu/jodie/{}.csv'.format(dataset)
            else:
                print("Dataset already in current repository please directly copy <Github.bin/SocialEvolve.bin> to the data folder")
                exit(-1)
        ...
        print("{} Dataset is exist, directly loaded.".format(dataset))
...
def TemporalGithubDataset():
    return TemporalDataset('Github')

def TemporalSocialEvolveDataset():
    return TemporalDataset('SocialEvolve')

def TemporalMoocDataset():
    return TemporalDataset('mooc')
